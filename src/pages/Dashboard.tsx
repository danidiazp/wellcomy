...
import {
  ArrowRight, FileText, Calendar, MapPin, BookOpen, Sparkles, AlertCircle,
  ListChecks, ExternalLink, Target, Bell, LayoutDashboard, CalendarClock, Lock,
} from "lucide-react";
...
const SECTIONS = [
  ...
  { id: "roadmap", label: "Mi roadmap", icon: MapPin },
...
// Card de upsell reutilizable para secciones bloqueadas según el plan.
const UpgradeCard = ({
  eyebrow, title, desc, ctaLabel, to,
}: { eyebrow: string; title: string; desc: string; ctaLabel: string; to: string }) => (
  <div className="rounded-3xl border border-border bg-card p-8 lg:p-12 shadow-elegant relative overflow-hidden">
...
const SectionTitle = ({ icon: Icon, title }: { icon: typeof MapPin; title: string }) => (
...
const Dashboard = () => {
  ...
  // Acceso por plan
  const canFullProcess = planTier === "base" || planTier === "premium";
  const canCitas = planTier === "premium";
  const sectionAllowed = (id: SectionId) =>
    id === "resumen" ? true : id === "citas" ? canCitas : canFullProcess;
  ...
  const upgradeToBase = (
    <UpgradeCard
      ...
      title="Desbloquea todo tu proceso"
      desc="Con el plan mensual accedes al roadmap completo por etapas, tu checklist documental personalizado, tus tareas y los recursos oficiales para cada paso."
      ctaLabel="Ver planes"
      to="/precios"
    />
  );
  const upgradeToPremium = (
    <UpgradeCard
      eyebrow="Plan Acompañamiento"
      title="Añade sesiones con un asesor"
      desc="El plan Acompañamiento incluye consultas de 30 minutos con un asesor especialista que revisa tu caso, además de todo tu proceso migratorio."
      ctaLabel="Mejorar a Acompañamiento"
      to="/precios"
    />
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-2xl sm:text-3xl font-semibold tracking-tight mt-1">Tu camino a España</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Diagnóstico del {new Date(assessment.created_at).toLocaleDateString("es-ES", { day: "numeric", month: "long", year: "numeric" })}
          </p>
        </div>
        <div className="flex gap-2">
          <Button asChild variant="outline" size="sm"><Link to="/diagnostico">Rehacer diagnóstico</Link></Button>
          <Button asChild variant="soft" size="sm"><Link to="/perfil">Editar perfil</Link></Button>
        </div>
      </div>

      {/* Upsell prueba → plan mensual */}
      {planTier === "trial" && (
        <div className="flex items-start gap-4 bg-primary/5 border border-primary/20 rounded-3xl p-5 lg:p-6">
          <div className="h-10 w-10 rounded-2xl bg-primary/10 grid place-items-center shrink-0">
            <Sparkles className="h-5 w-5 text-primary" />
          </div>
          <div className="space-y-2 min-w-0">
            <p className="font-display font-semibold text-sm">Estás en tu prueba gratuita</p>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Ahora ves tu próximo paso. Con el plan mensual desbloqueas el roadmap completo, tu checklist documental, tareas y recordatorios de todo el proceso.
            </p>
            <Button asChild variant="hero" size="sm" className="mt-1">
      ...
      {/* Fast-track nacionalidad */}
      {assessment.eligible_fast_track_nationality && (
        <div className="flex items-start gap-4 bg-accent/10 border border-accent/30 rounded-3xl p-5 lg:p-6">
          ...
          <div className="space-y-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Como nacional de un país iberoamericano, podrás pedir la nacionalidad española tras <strong>2 años de residencia legal</strong> (en lugar de 10 del régimen general).{" "}
          ...
          <div className="relative grid lg:grid-cols-[1.5fr_1fr] gap-6 items-center">
            <div className="space-y-3">
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-2">
                ...
                {canFullProcess && route && (
                  <Button variant="outline" size="sm" onClick={() => setSection("roadmap")} className="bg-transparent border-primary-foreground/30 text-primary-foreground hover:bg-primary-foreground/10 hover:text-primary-foreground">
  ...
  const RoadmapSection = !canFullProcess ? upgradeToBase : (
    <div className="space-y-6">
      <SectionTitle icon={MapPin} title="Mi roadmap" />
      {steps.length > 0 ? (
        <>
          ...
          <div className="bg-card border border-border rounded-3xl p-6 lg:p-8 shadow-elegant">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-display text-lg font-semibold">Etapas de tu ruta</h3>
        ...
        <p className="text-sm text-muted-foreground">Aún no hay etapas asignadas a tu ruta.</p>
      )}
    </div>
  );

  const DocumentosSection = !canFullProcess ? upgradeToBase : (
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-3xl p-6 lg:p-7 shadow-elegant">
        ...
        {user && (
          <DocumentChecklist profileId={user.id} docs={docs} states={docStates} onChange={setDocStates} />
        )}
      </div>
    </div>
  );

  const TareasSection = !canFullProcess ? upgradeToBase : (
    <div className="space-y-6">
      <SectionTitle icon={ListChecks} title="Tareas" />
      <div className="bg-card border border-border rounded-3xl p-6 lg:p-7 shadow-elegant">
        {user && (
          <TaskList
            profileId={user.id}
  ...
  const RecursosSection = !canFullProcess ? upgradeToBase : (
    <div className="space-y-6">
      <SectionTitle icon={BookOpen} title="Recursos" />
      <div className="bg-card border border-border rounded-3xl p-6 lg:p-7 shadow-elegant">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-display text-lg font-semibold">Recursos oficiales para tu ruta</h3>
          <Button asChild variant="ghost" size="sm"><Link to="/recursos">Ver biblioteca <ArrowRight className="h-3.5 w-3.5" /></Link></Button>
        </div>
        {resources.length > 0 ? (
          ...
          <p className="text-sm text-muted-foreground">Aún no hay recursos específicos para tu ruta. Visita la biblioteca general.</p>
        )}
      </div>
    </div>
  );

  const CitasSection = !canCitas ? upgradeToPremium : (
    <div className="space-y-6">
      <SectionTitle icon={CalendarClock} title="Citas" />
  ...
  const sectionContent: Record<SectionId, JSX.Element> = {
    ...
    tareas: TareasSection,
    recursos: RecursosSection,
    citas: CitasSection,
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      <SiteHeader />
      <div className="flex-1 w-full container max-w-7xl py-6 lg:py-8">
          ...
          <aside className="lg:w-56 shrink-0">
            <nav className="flex lg:flex-col gap-1 overflow-x-auto lg:overflow-visible pb-1 lg:pb-0 lg:sticky lg:top-24">
              {SECTIONS.map((s) => {
                ...
                const locked = !sectionAllowed(s.id);
                return (
                  <button
                    key={s.id}
